...
export default class AppModel {
    ...
    getSuggestionItems(indexKey, filter){
        const set = this.suggestionIndexes[indexKey];
        let items = set ? set.items : [];
        return items;
    }

    toggleSearchCriteriaByKey(indexKey, key) {
        const item = this.findCriteria(indexKey, key);
        if (!item)
           return ;
        return this.toggleSearchCriteria(item);
    }

    toggleSearchCriteria(item){ 
    ...
    findCriteria(indexKey, key) {
        const set = this.suggestionIndexes[indexKey];
        if (!set) return ;
        const item = set.find(function(r){
